...
/* just free() allocated memory - destructor */
...
void 
hash_apply_f (struct hash *h, char* buf) {
    hash_action_func *f;
    if (!strcmp(buf,"square")) {
    	f = hash_action_square;
    }
    else if (!strcmp(buf,"triple")) {
        f = hash_action_triple;
    }
    hash_apply(h, f);
}
...
/* Dumpdata: iterate through Hash table */
void
hash_dumpdata (struct hash *h) {
  struct hash_iterator i;
  hash_first(&i, h);
  while(hash_next(&i)) {
...
/* Returns a hash of integer I. manual version. */
unsigned 
hash_int_2 (int i) {
  unsigned hash;
