...
/* Resize B to be size of BIT_CNT bits
   and sets newer bits to false.
   Returns struct bitmap* if success, NULL if memory allocation
   failed.  
   if B != NULL and size == 0, then B will be freed, NULL be returned. */
struct bitmap *
bitmap_expand (struct bitmap* b, int size) {
  if (b != NULL) {
    size_t size_bef = b -> bit_cnt; // original size
    ASSERT (size_bef <= size);
    b -> bits = realloc(b->bits, byte_cnt(size));
    if (b->bits != NULL)
    {
      b -> bit_cnt = size;
      bitmap_set_multiple (b, size_bef, size-size_bef, false);
      return b;
    }
  }
  return NULL;
...
/* Print dumpdata of B from start to end */
void
bitmap_dumpdata (const struct bitmap *b) {
